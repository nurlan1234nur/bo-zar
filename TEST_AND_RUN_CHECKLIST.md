...
```powershell
cd "C:\Users\Nurlan's Victus\Documents\new"
```
...
```text
USER  phone: 99112233  password: password123
ADMIN phone: 99000000  password: admin123
```
...
```powershell
$env:EXPO_PUBLIC_API_BASE_URL="http://YOUR_COMPUTER_LAN_IP:8080/api/v1"
corepack pnpm --dir mobile-app dev
```
...
$login = Invoke-RestMethod `
  ...
  -Body '{"identifier":"99112233","password":"password123"}'
...
Invoke-RestMethod `
  ...
  -Uri http://localhost:8080/api/v1/auth/login `
  -ContentType "application/json" `
  -Body '{"identifier":"99112233","password":"password1234"}'
```

Important:

- If you change demo password, change it back to `password123` or reseed DB.
...
Invoke-RestMethod `
  ...
  -ContentType "application/json" `
  -Body "{`"identifier`":`"99112233`",`"resetToken`":`"$($reset.data.resetToken)`",`"newPassword`":`"password123`"}"
```
...
$ad = Invoke-RestMethod `
  ...
  -Headers @{ Authorization = "Bearer $token" } `
  -ContentType "application/json" `
  -Body '{"title":"Test zar","description":"Test description from checklist","price":1000,"categoryId":1,"locationId":1,"contactPhone":"99112233"}'
